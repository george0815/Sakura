...
//Jump, sets the PC to a new value, allowing code to execute from a new location 
//JMP BUG: if the new 2 byte PC crosses a page (ends in FF), then the CPU fails to increment the page wheen reading the second byte
...
//Pushes the current program counter to the stack, then sets the PC to a new value
//then the code can call a function and return using RTS
...
//Logical shift right, shifts all bits of a memory value or the accumulator one position to the right
//0 is shifted into bit 7, and 0 is shifted to the carry flag
//equivalent of dividing an unsigned integer by 2 and rounding down, with the remainder in the carry flag
...
//No operation, literally just does fucking nothing and wastes space and CPU cycles 
...
//Push processor status, pushes a byte representing the status register to the stack, then decrements the stack pointer 
//the B flag and the extra bit are both pushed as one
...
//Pull A, increments the stack pointer then loads that value at that stack position into A
...
//Pull processor status, increments the stack pointer then loads that byte into the 6 flags
...
//Return from interrupt, returns from an interrupt handler by pulling the status flags from the stack, then pulling the new program counter
//similar to PLP but changes to the interrupt disable flag apply immediately instead of being delayed one instruction
...
//Return from subroutine, pulls an address from the stack into the program counter, then increments the program counter
...
//Subtract with carry, subtracts a memory value and the NOT of the carry flag from the accumulator 
...
//Transfer X to the stack pointer, copies the X register value to the stack pointer
